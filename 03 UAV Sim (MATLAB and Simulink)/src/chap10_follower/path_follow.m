...
function out = path_follow(in)
    ...
    chi_infty = 45*pi/180;  % approach angle for large distance from straight-line path
    k_path    = 0.05;       % proportional gain for path following
    k_orbit   = 7.5;       % proportional gain for orbit following
    gravity   = 9.81;
    ...
    switch flag
        case 1 % follow straight line path specified by r and q
            ...
            % commanded altitude
            h_c = -r_path(3) - (sqrt(r_path(1)^2 + r_path(2)^2)*q_path(3)/sqrt(q_path(1)^2+q_path(2)^2));
        ...
        case 2 % follow orbit specified by c, rho, lam
            ...
            while (varphi - chi > +pi), varphi = varphi - 2*pi; end
             % compute orbit error
            orbit_error = (d - rho_orbit)/rho_orbit;
            % heading command
            chi_c = varphi + lam_orbit*(0.5*pi+atan(k_orbit*orbit_error));
