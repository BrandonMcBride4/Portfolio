...
function dubinspath = hisDubinsParameters(start_node, end_node, R)
...
%       L2 = sqrt(ell^2-4*R^2) + R*mod(2*pi+mod(theta-theta2,2*pi)-mod(chis-pi/2,2*pi),2*pi)...
%           +R*mod(2*pi+mod(theta2+pi,2*pi)-mod(chie+pi/2,2*pi),2*pi);
    ...
    if isreal(theta2)==0
      ...
      L3 = sqrt(ell^2-4*R^2) + R*mod(2*pi-mod(theta+theta2,2*pi) + mod(chis+pi/2,2*pi),2*pi)...
          +R*mod(2*pi-mod(theta+theta2-pi,2*pi)+mod(chie-pi/2,2*pi),2*pi);
    end
    % compute L4
    theta = atan2(cle(2)-cls(2),cle(1)-cls(1));
    L4 = norm(cls-cle) + R*mod(2*pi-mod(theta+pi/2,2*pi)+mod(chis+pi/2,2*pi),2*pi)...
        +R*mod(2*pi-mod(chie+pi/2,2*pi)+mod(theta+pi/2,2*pi),2*pi);
    ...
    switch(idx)
        ...
        case 3
            ...
            w2 = ce + R*rotz(-pi+theta+theta2)*e1;
         case 4
            cs = cls;
            lams = -1;
            ce = cle;
