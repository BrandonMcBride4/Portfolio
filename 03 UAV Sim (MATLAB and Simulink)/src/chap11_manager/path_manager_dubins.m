% path_manager_dubins 
%   - follow Dubins paths between waypoint configurations
%
% Modified:  
%   - 4/1/2010 - RWB
%   - 3/198/2019 - RWB
%
% input is:
%   num_waypoints - number of waypoint configurations
%   waypoints    - an array of dimension 5 by PLAN.size_waypoint_array.
%                - the first num_waypoints rows define waypoint
%                  configurations
%                - format for each waypoint configuration:
%                  [wn, we, wd, chi_d, Va_d]
%                  where the (wn, we, wd) is the NED position of the
%                  waypoint, chi_d is the desired course at the waypoint,
%                  and Va_d is the desired airspeed along the path. 
%
% output is:
%   flag - if flag==1, follow waypoint path
%          if flag==2, follow orbit
%   
%   Va^d - desired airspeed
%   r    - inertial position of start of waypoint path
%   q    - unit vector that defines inertial direction of waypoint path
%   c    - center of orbit
%   rho  - radius of orbit
%   lambda = direction of orbit (+1 for CW, -1 for CCW)
%
function out = path_manager_dubins(in,PLAN,start_of_simulation)
  persistent wp_idx;
  persistent dubins_state;

  NN = 0;
  num_waypoints = in(1+NN);
  waypoints = reshape(in(2+NN:5*PLAN.size_waypoint_array+1+NN),5,PLAN.size_waypoint_array);
  NN = NN + 1 + 5*PLAN.size_waypoint_array;
  pn        = in(1+NN);
  pe        = in(2+NN);
  h         = in(3+NN);
  % Va      = in(4+NN);
  % alpha   = in(5+NN);
  % beta    = in(6+NN);
  % phi     = in(7+NN);
  % theta   = in(8+NN);
  chi     = in(9+NN);
  % p       = in(10+NN);
  % q       = in(11+NN);
  % r       = in(12+NN);
  % Vg      = in(13+NN);
  % wn      = in(14+NN);
  % we      = in(15+NN);
  % psi     = in(16+NN);
  state     =  in(1+NN:16+NN);
  NN = NN + 19;
  t         = in(end);
  
  if start_of_simulation
      wp_idx = 2;
      dubins_state = 2;
  end
  p = [pn; pe; -h];
  Va_d = waypoints(5, wp_idx-1);
  
  start_node = waypoints(:, wp_idx-1)';
  end_node = waypoints(:, wp_idx)';
  
  dubinspath = dubinsParameters(start_node, end_node, PLAN.R_min);
  switch dubins_state
      case 1
          flag = 2;
          r = dubinspath.w1; % Not used in orbit
          q = dubinspath.q1; % Not used in orbit
          c = dubinspath.cs;
          rho = PLAN.R_min;
          lambda = dubinspath.lams;
          if dot(p-dubinspath.w1, -dubinspath.q1) >= 0
              dubins_state = 2;
          end
      case 2
          flag = 2;
          r = dubinspath.w1; % Not used in orbit
          q = dubinspath.q1; % Not used in orbit
          c = dubinspath.cs;
          rho = PLAN.R_min;
          lambda = dubinspath.lams;
          if dot(p-dubinspath.w1, dubinspath.q1) >= 0
              dubins_state = 3;
          end
      case 3
          flag = 1;
          r = dubinspath.w1;
          q = dubinspath.q1;
          c      = [0;0;0];    % not used for waypoint path
          rho    = 0;          % not used for waypoint path
          lambda = 0;          % not used for waypoint path
          if dot(p-dubinspath.w2, dubinspath.q1) >= 0
              dubins_state = 4;
          end
      case 4
          flag = 2;
          r = dubinspath.w1; % Not used in orbit
          q = dubinspath.q1; % Not used in orbit
          c = dubinspath.ce;
          rho = PLAN.R_min;
          lambda = dubinspath.lame;
          if dot(p-dubinspath.w3, -dubinspath.q3) >= 0
              dubins_state = 5;
          end
      case 5
          flag = 2;
          r = dubinspath.w1; % Not used in orbit
          q = dubinspath.q1; % Not used in orbit
          c = dubinspath.ce;
          rho = PLAN.R_min;
          lambda = dubinspath.lame;
          if dot(p-dubinspath.w3, dubinspath.q3) >= 0
              dubins_state = 1;
              wp_idx = wp_idx + 1; 
          end
  end
  
  if wp_idx < num_waypoints - 1
      flag_need_new_waypoints = 0;
  else
      flag_need_new_waypoints = 1;
  end
  
  out = [flag; Va_d; r; q; c; rho; lambda; state; flag_need_new_waypoints];

end