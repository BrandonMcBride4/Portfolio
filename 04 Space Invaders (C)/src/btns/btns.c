...
#define GIER_ENABLE 0x80000000 // 31st bit needs to be 1 to enable interrupts
...
int32_t btns_init(char devDevice[])
{
    ...
    btns_write(GPIO_TRI_OFFSET, GPIO_TRI_INPUT); // Says we read from the buttons
    btns_write(IP_IER_OFFSET, IP_IER_ENABLE);    // Enables interrupts for the buttons
    btns_write(GIER_OFFSET, GIER_ENABLE);        // Enables interrupts for the buttons
...
uint32_t btns_read_gpio(uint32_t offset)
{
...
{
    btns_write(IP_ISR_OFFSET, IP_ISR_ENABLE);
}
